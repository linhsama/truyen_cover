...
class TuongTacModel extends Database
{
    ...
    function TuongTac__Check_Exist($tai_khoan_id, $tuong_tac_loai)
    {
        $sql = "SELECT COUNT(*) as count FROM tuong_tac WHERE tai_khoan_id = ? AND tuong_tac_loai = ?";
        $obj = $this->connect->prepare($sql);
        $obj->execute([$tai_khoan_id, $tuong_tac_loai]);
    ...
    function TuongTac__AddOrUpdate($tuong_tac_noi_dung, $tuong_tac_loai, $chapter_id, $tai_khoan_id)
    {
        ...
        $kiemTraTuongTac = $this->TuongTac__Check_Exist($tai_khoan_id, $tuong_tac_loai);
