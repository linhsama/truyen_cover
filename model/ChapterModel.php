...
class ChapterModel extends Database
{
    ...
    {
        // Xóa chapter_so 3 nếu nó tồn tại
        $stmtDelete = $this->connect->prepare("DELETE FROM chapter WHERE truyen_id = ? AND chapter_so = ?");
        $stmtDelete->execute(array($truyen_id, 3));

        // Cập nhật lại thứ tự của chapter_so sau mỗi lần xóa
